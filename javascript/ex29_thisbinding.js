...
console.log(printName.apply({ name: '송명보' }));
...
// call => ,(콤마 , 쉼표) 기반으로 매게 인자값을 this 다음에 순서대로 넘겨준다
// apply => 배열을 기반으로 인자값을 넘겨준다

const dog = {
  name: '멍멍이'
}
// bind() => 내가 원할 때 실행한다 => this값만 우리가 원하는 상태로 함수 세팅해논 것

const printCatName = printName.call(cat); //즉시 함수를 호출해서실행 => return 결과값 저장
const printDogName = printName.bind(dog); // this dog로 바인딩한 함수를 저장 ==> 우리가 원할 때 호출해서 실행할 수 있다.
